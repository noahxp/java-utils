...
public class SlackManager extends AbstractManager {
  ...
  /* Priority define */
  private static SlackField errorFields = new SlackField("Priority", "High");
  private static SlackField warnFields = new SlackField("Priority", "Medium");
  private static SlackField infoFields = new SlackField("Priority", "Low");
  ...
  public void sendMessage(final Layout<?> layout, final LogEvent event) {
    ...
    if (channel == null || channel.trim().length() == 0) {
      throw new AppenderLoggingException("no slack channel provided for SlackAppender");
    }
    ...
    Runnable runnable = () -> {
      ...
      try {
        ...
        if (proxyUrl != null && proxyUrl.trim().length() > 0 && proxyPort > 0) {
          ...
          conn = (HttpURLConnection) webHook.openConnection(proxy);
        } else {
